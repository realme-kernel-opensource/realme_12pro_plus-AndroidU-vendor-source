...
// 2021/10/08, Add for speaker mute
...
int sia81xx_volme_boost_set(struct oplus_speaker_device *speaker_device, int level);
...
void sia81xx_speaker_mute_set(int enable)
{
	...
	list_for_each_entry(si_pa, &sia81xx_list, list) {
		if(sia81xx_speaker_channel_num == si_pa->channel_num) {
			break;
		}
	}
...
#ifdef CONFIG_SND_SOC_OPLUS_PA_MANAGER
	if (get_speaker_manufacturer(L_SPK) != MFR_AWINIC) {
		/* get chip type value,
...
#ifdef CONFIG_SND_SOC_OPLUS_PA_MANAGER
	if (!IS_DIGITAL_PA_TYPE(si_pa->chip_type)) {
		spk_dev_node = si_pa->oplus_dev_node;
