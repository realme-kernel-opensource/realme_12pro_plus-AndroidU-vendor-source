	...
	if(oplus_vooc_get_fastchg_started() == true) {
		...
		if(chip->fastchg_ffc_status == 2 || chip->fastchg_ffc_status == 1) {
			...
			if (chip->tbatt_status == BATTERY_STATUS__COLD_TEMP) {
				full_vol = chip->limits.cold_vfloat_sw_limit;
			} else if (chip->tbatt_status == BATTERY_STATUS__LITTLE_COLD_TEMP) {
				full_vol = chip->limits.little_cold_vfloat_sw_limit;
			} else if (chip->tbatt_status == BATTERY_STATUS__COOL_TEMP) {
				full_vol = chip->limits.cool_vfloat_sw_limit;
				...
				full_vol = chip->limits.warm_vfloat_sw_limit;
			}
			if ((!chip->authenticate) || (!chip->hmac)) {
				full_vol = chip->limits.non_standard_vfloat_sw_limit;
			}
			if (oplus_short_c_batt_is_prohibit_chg(chip)) {
				full_vol = chip->limits.short_c_bat_vfloat_sw_limit;
			}
			full_curr = chip->limits.iterm_ma;
		}
	}
...
int oplus_chg_get_battery_btb_temp_cal(void)
{
	...
	if (!IS_ERR_OR_NULL(bcdev->iio.batt0_con_btb_chan)) {
		...
		chg_info("[OPLUS_CHG]: batt0_con_btb_chan is NULL !\n");
	}
