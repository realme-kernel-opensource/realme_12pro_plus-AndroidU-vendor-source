...
static void set_sched_boost(struct task_struct *p, bool enable)
{
	...
	if (enable) {
		...
		oplus_set_ux_state_lock(p, (ux_state | UX_PRIORITY_AUDIO | SA_TYPE_HEAVY), true);
	} else {
		oplus_set_ux_state_lock(p, (ux_state & ~(SCHED_ASSIST_UX_PRIORITY_MASK | SA_TYPE_HEAVY)), true);
	}
