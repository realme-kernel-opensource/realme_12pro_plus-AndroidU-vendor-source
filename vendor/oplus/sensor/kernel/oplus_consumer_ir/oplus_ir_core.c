...
static int parse_hw_core_config(struct device *dev, struct ir_core* ir_core)
{
	...
	if (!ir_core) {
		...
		if (ir_core->core_config.vdd_type == IR_GPIO_VDD_TYPE) {
			...
			ir_core->core_config.ir_en_gpio = of_get_named_gpio(np, "ir-en", 0);
			if (ir_core->core_config.ir_en_gpio < 0) {
				pr_err("oplus_ir_core: get ir_en_gpio fail\n");
			}
		} else if (ir_core->core_config.vdd_type == IR_EXTERNAL_VDD_TYPE) {
			ir_core->core_config.vdd_3v0 = NULL;
...
static void enable_ir_vdd(struct ir_core *ir_core)
{
	int retval = 0;
	if (ir_core->core_config.vdd_type == IR_GPIO_VDD_TYPE &&
		gpio_is_valid(ir_core->core_config.ir_en_gpio)) {
		gpio_set_value(ir_core->core_config.ir_en_gpio, 1);
		pr_info("oplus_ir_core: %s\n", __func__);
	} else if (ir_core->core_config.vdd_3v0 != NULL) {
		regulator_set_voltage(ir_core->core_config.vdd_3v0,
			ir_core->core_config.vdd_min_vol, ir_core->core_config.vdd_max_vol);
...
static void disable_ir_vdd(struct ir_core *ir_core)
{
	if (ir_core->core_config.vdd_type == IR_GPIO_VDD_TYPE &&
		gpio_is_valid(ir_core->core_config.ir_en_gpio)) {
		gpio_set_value(ir_core->core_config.ir_en_gpio, 0);
		pr_info("oplus_ir_core: %s\n", __func__);
	} else if (ir_core->core_config.vdd_3v0 != NULL) {
...
static int ir_core_probe(struct platform_device *pdev)
{
	...
	if (gpio_is_valid(g_ir->core_config.ir_en_gpio)) {
		ret = devm_gpio_request_one(&pdev->dev, g_ir->core_config.ir_en_gpio,
			GPIOF_OUT_INIT_LOW, "ir_en");
		if (ret < 0) {
			pr_err("Failed to request ir en\n");
			goto err_iren_config;
		}
	}
	return 0;
...
err_iren_config:
	if (gpio_is_valid(g_ir->core_config.ir_en_gpio))
		devm_gpio_free(&pdev->dev, g_ir->core_config.ir_en_gpio);
	return ret;
}
