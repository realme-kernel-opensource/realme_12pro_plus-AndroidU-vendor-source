	...
	struct device *dev)
{
	...
	rc = of_property_read_u32(of_node, "is_need_flush_fireware", &a_ctrl->is_need_flush_fireware);
	if (rc)
	{
		a_ctrl->is_need_flush_fireware = 0;
		CAM_ERR(CAM_ACTUATOR, "get failed for is_need_flush_fireware = %d",a_ctrl->is_need_flush_fireware);
	}
	else
	{
		CAM_INFO(CAM_ACTUATOR, "read is_need_flush_fireware success, value:%d", a_ctrl->is_need_flush_fireware);
	}
#endif
