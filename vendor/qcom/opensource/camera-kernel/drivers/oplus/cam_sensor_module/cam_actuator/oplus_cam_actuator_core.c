...
#define DW9827C_STANDBY_ADDR 0x02
#define DW9827C_STANDBY_DATA 0x40
#define DW9827C_SAL_FLAG 0x95
#define DS9827C_SAL_ON 0x01
...
uint32_t DW9827C_SALOFF[ACTUATOR_REGISTER_SIZE][2] = {
	{0x34, 0x85},
	{0x95, 0x00},
	{0x03, 0x01},
	{0xff, 0xff},
...
uint32_t DW9827C_SWRESET[ACTUATOR_REGISTER_SIZE][2] = {
	{0x34, 0x52},
	{0x04, 0x01},
	{0xff, 0xff},
};
...
void oplus_cam_actuator_flush_fireware(struct cam_actuator_ctrl_t *a_ctrl)
{
	uint32_t read_val = 0;
	int ret = 0;
	int i;
	...
	ret = cam_actuator_ramwrite(a_ctrl,
					DW9827C_STANDBY_ADDR,
					DW9827C_STANDBY_DATA,
					1,
					CAMERA_SENSOR_I2C_TYPE_BYTE,
					CAMERA_SENSOR_I2C_TYPE_BYTE);
	if(ret < 0) {
		CAM_ERR(CAM_ACTUATOR, "oplus_cam_actuator_flush_fireware set Actutor Standy mode failed");
	}
	mdelay(3);
	ret = camera_io_dev_read(&(a_ctrl->io_master_info),
		...
		CAMERA_SENSOR_I2C_TYPE_BYTE);
	if (ret < 0) {
		CAM_ERR(CAM_ACTUATOR, "oplus_cam_actuator_flush_fireware read value failed");
	} else {
		CAM_ERR(CAM_ACTUATOR, "oplus_cam_actuator_flush_fireware read value %d", read_val);
		if (read_val == DS9827C_SAL_ON) {
			// start flush fireware
			for (i = 0 ; i < ACTUATOR_REGISTER_SIZE; i++) {
				if ( (DW9827C_SALOFF[i][0] != 0xff) && (DW9827C_SALOFF[i][1] != 0xff) ) {
					ret = cam_actuator_ramwrite(a_ctrl,
						(uint32_t)DW9827C_SALOFF[i][0],
						(uint32_t)DW9827C_SALOFF[i][1],
						1,
						CAMERA_SENSOR_I2C_TYPE_BYTE,
						CAMERA_SENSOR_I2C_TYPE_BYTE);
					if(ret < 0)
			...
			// sw reset
			for (i = 0 ; i < ACTUATOR_REGISTER_SIZE; i++) {
				if ((DW9827C_SWRESET[i][0] != 0xff) && (DW9827C_SWRESET[i][1] != 0xff)) {
					ret = cam_actuator_ramwrite(a_ctrl,
						(uint32_t)DW9827C_SWRESET[i][0],
						(uint32_t)DW9827C_SWRESET[i][1],
						1,
						CAMERA_SENSOR_I2C_TYPE_BYTE,
						CAMERA_SENSOR_I2C_TYPE_BYTE);
					if(ret < 0)
			...
			CAM_ERR(CAM_ACTUATOR,"no need flush fireware");
		}
	}
}
