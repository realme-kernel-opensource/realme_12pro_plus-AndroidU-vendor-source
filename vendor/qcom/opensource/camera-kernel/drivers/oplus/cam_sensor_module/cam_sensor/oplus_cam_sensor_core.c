		...
		else if (s_ctrl->sensordata->slave_info.sensor_id == 0x0615)
		{
			...
			else
			{
				...
				else if(s_ctrl->sensordata->id_info.sensor_id > IMX615_FAB2_SENSOR_ID && vendor_id > IMX615_FAB2_VENDOR_ID)
				{
					...
					sensor_setting.addr_type = sensor_settings.imx615_setting_fab2.addr_type;
					sensor_setting.data_type = sensor_settings.imx615_setting_fab2.data_type;
					sensor_setting.size = sensor_settings.imx615_setting_fab2.size;
					sensor_setting.delay = sensor_settings.imx615_setting_fab2.delay;
					rc = camera_io_dev_write(&(s_ctrl->io_master_info), &sensor_setting);
				}
			}
		}
...
void cam_sensor_get_dt_data(struct cam_sensor_ctrl_t *s_ctrl)
{
		...
		rc = of_property_read_u32(of_node, "power_down_delay",&s_ctrl->power_down_delay);
		if ( rc < 0)
		{
			CAM_DBG(CAM_SENSOR, "Invalid sensor params");
			s_ctrl->power_down_delay = 0;
		}
		else
		{
			CAM_INFO(CAM_SENSOR, "get power_down_delay succeed  value is %d",s_ctrl->power_down_delay);
		}
		...
		if(s_ctrl->sensor_initsetting_state == CAM_SENSOR_SETTING_WRITE_INVALID)
		{
			...
			else if(s_ctrl->sensordata->slave_info.sensor_id == 0x1634)
			{
				if(s_ctrl->power_up_advance == 1)
				{
					...
					if(rc < 0)
					{
						CAM_ERR(CAM_SENSOR, "write 1634 coffeeA setting failed!");
					}
					else
					{
						CAM_INFO(CAM_SENSOR, "write 1634 coffeeA setting success!");
						s_ctrl->sensor_initsetting_state = CAM_SENSOR_SETTING_WRITE_SUCCESS;
					}
				}
			}
			else if(s_ctrl->sensordata->slave_info.sensor_id == 0x8202)
			{
				if(s_ctrl->power_up_advance == 1)
				{
					...
					if(rc < 0)
					{
						CAM_ERR(CAM_SENSOR, "write 882 coffeeA setting failed!");
					}
					else
					{
						CAM_INFO(CAM_SENSOR, "write 882 coffeeA setting success!");
						s_ctrl->sensor_initsetting_state = CAM_SENSOR_SETTING_WRITE_SUCCESS;
					}
					if(rc == 0 && s_ctrl->sensor_qsc_setting.enable_qsc_write_in_advance && s_ctrl->sensor_qsc_setting.read_qsc_success)
					{
						...
						if(rc < 0)
						{
							CAM_ERR(CAM_SENSOR, "write 882 coffeeA qsc failed!");
						}
						else
						{
							CAM_INFO(CAM_SENSOR, "write 882 coffeeA qsc success!");
...
int cam_sensor_disable_regulator(struct cam_sensor_ctrl_t *s_ctrl)
{
	int rc = 0;
	...
	CAM_ERR(CAM_SENSOR,"sensor status is %d", s_ctrl->sensor_iodd_status);

	if (s_ctrl->sensor_iodd_status == CAM_SENSOR_ON
		&& ((s_ctrl->sensordata->slave_info.sensor_id == 0x6442)
		|| (s_ctrl->sensordata->slave_info.sensor_id == 0x0890)
		|| (s_ctrl->sensordata->slave_info.sensor_id == 0x8202)))
	{
		rc =  cam_soc_util_regulator_disable(
				soc_info->rgltr[0],
				soc_info->rgltr_name[0],
				soc_info->rgltr_min_volt[0],
				soc_info->rgltr_max_volt[0],
				soc_info->rgltr_op_mode[0],
				soc_info->rgltr_delay[0]);
		if (rc < 0)
		{
			CAM_ERR(CAM_SENSOR,"sensor do stop falied");
		}
		else
		{
			s_ctrl->sensor_iodd_status = CAM_SENSOR_OFF;
			CAM_INFO(CAM_SENSOR,"sensor do stop succeed");
		}
	}
