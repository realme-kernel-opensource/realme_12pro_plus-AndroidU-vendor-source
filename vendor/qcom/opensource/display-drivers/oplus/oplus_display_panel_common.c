...
bool oplus_ffc_handle = false;
EXPORT_SYMBOL(oplus_ffc_handle);
static int last_refresh_rate;
...
int oplus_display_panel_set_dynamic_osc_clock(void *data)
{
	...
	if (!rc) {
		mutex_lock(&display->panel->panel_lock);
		if (!strcmp(display->panel->name, "coffee tianma ili7838a dsc cmd mode panel"))
			display->panel->oplus_priv.ffc_changed = true;
		else
...
int oplus_panel_ffc_handle(void *sde_encoder_virt)
{
	int rc = 0;
	...
	if (display->panel->oplus_priv.ffc_changed == false) {
		OFP_DEBUG("no need to handle ffc\n");
		last_refresh_rate = display->panel->cur_mode->timing.refresh_rate;
		return 0;
	}

	OPLUS_OFP_TRACE_BEGIN("oplus_ofp_ffc_handle");

	refresh_rate = display->panel->cur_mode->timing.refresh_rate;
	us_per_frame = 1000000/refresh_rate;
	delay_us = (us_per_frame >> 1) + 700;

	OFP_INFO("last_refresh_rate=%d,refresh_rate=%d,delay_us=%u\n", last_refresh_rate, refresh_rate, delay_us);
	if (last_refresh_rate != refresh_rate)
	...
	mutex_unlock(&display->panel->panel_lock);
	if (!rc) {
		display->panel->oplus_priv.ffc_changed = false;
	}

	OPLUS_OFP_TRACE_END("oplus_ofp_ffc_handle");
...
int oplus_panel_parse_ffc_config(struct dsi_panel *panel)
{
	...
	panel->oplus_priv.ffc_handle = utils->read_bool(utils->data,
			"oplus,ffc-handle");
	if (panel->oplus_priv.ffc_enabled) {
		oplus_ffc_handle = true;
	}

	panel->oplus_priv.ffc_changed = false;
